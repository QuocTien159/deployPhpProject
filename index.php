<?php
// index.php – Controller trung tâm
...
if($action === 'list'){
    $q = $_GET['q'] ?? ''; // tìm kiếm
    $filterMalop = ($_GET['malop'] ?? '') !== '' ? $_GET['malop'] : null; // lọc lớp
    ...
    $total       = $svModel->count($q, $filterMalop); // tổng số sv
    $totalPages  = max(1, ceil($total / $limit)); // tổng số trang
    $students    = $svModel->getAll($limit, $offset, $q, $filterMalop); // ds sv
    $lops        = $lopModel->all(); // ds lớp
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if(empty($_POST['masv']) || empty($_POST['tensv']) || empty($_POST['malop'])){
            ...
            try {
                ...
                header("Location: index.php"); // quay về list
                exit;
